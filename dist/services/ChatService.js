 ...
 */
async function getPlayerChatInfo(playerId) {
 ...
 */
async function findPlayerInRoom(name, roomId, excludePlayerId) {
    ...
    const prefixMatch = candidates.find((p) => p.name.toLowerCase().startsWith(nameLower));
    if (prefixMatch) {
 ...
 */
export async function speak(playerId, message) {
 ...
 */
export async function shout(playerId, message) {
    const playerInfo = await getPlayerChatInfo(playerId);
    if (!playerInfo) {
        return { success: false, error: "Player not found or not in a room" };
    }
 ...
 */
export async function whisper(playerId, targetName, message) {
    const playerInfo = await getPlayerChatInfo(playerId);
    if (!playerInfo) {
        return { success: false, error: "Player not found or not in a room" };
    }
    ...
    const target = await findPlayerInRoom(targetName, playerInfo.roomId, playerId);
    if (!target) {
        return {
            success: false,
 ...
 * @returns ChatResult with message data and room scope
 */
export async function emote(playerId, action) {
    const playerInfo = await getPlayerChatInfo(playerId);
    if (!playerInfo) {
        return { success: false, error: "Player not found or not in a room" };
    }
