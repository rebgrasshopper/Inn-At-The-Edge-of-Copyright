 ...
 */
export function findSimilarVerb(input) {
    ...
    switch (command.type) {
        ...
        default:
            ...
            if (command.action && command.target) {
                const feature = await FeatureService.findFeatureByCommand(room.id, command.action, command.target);
                if (feature) {
 ...
 */
async function executeMovement(command, context) {
    const { player } = context;
    if (!command.target) {
        return { success: false, message: "Which direction do you want to go?" };
    }
    const direction = command.target;
 ...
 */
async function executeChat(command, context) {
    const { player } = context;
    switch (command.action) {
        ...
        case "whisper": {
            ...
            const result = await ChatService.whisper(player.id, command.target, message);
            if (!result.success) {
                return { success: false, message: result.error };
            }
            const broadcasts = [];
 ...
 */
async function executeItem(command, context) {
    const { player, room } = context;
    ...
    switch (command.action) {
        case "get": {
            ...
            if (fromMatch) {
                ...
                const result = await ItemService.getItemFromContainer(player.id, room.id, itemName.trim(), containerName.trim(), quantity);
                return { success: result.success, message: result.message };
            }
            const result = await ItemService.getItem(player.id, room.id, command.target, quantity);
            return { success: result.success, message: result.message };
        }
        case "drop": {
            if (!command.target) {
            ...
            const result = await ItemService.dropItem(player.id, room.id, command.target, quantity);
            return { success: result.success, message: result.message };
        }
        case "examine": {
            if (!command.target) {
            ...
            const result = await ItemService.examineItem(player.id, room.id, itemName, searchOwn);
 ...
 */
async function executeInfo(command, context) {
    const { player, room } = context;
    switch (command.action) {
        case "look": {
            // Get fresh room data
 ...
 */
async function executeFeature(command, context) {
    const { player, room } = context;
    if (!command.target) {
 ...
 */
async function executeFeatureInteraction(playerId, feature) {
