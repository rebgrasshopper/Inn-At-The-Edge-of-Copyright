...
import { containerInventory, containers, items, playerInventory, roomInventory, } from "../db/schema.js";
/**
 ...
 */
async function findItemInRoom(roomId, itemName) {
    ...
    const exactMatch = roomItems.find((r) => r.items.name.toLowerCase() === nameLower ||
    ...
    // Try prefix match
    const prefixMatch = roomItems.find((r) => r.items.name.toLowerCase().startsWith(nameLower) ||
 ...
 */
async function findItemInPlayerInventory(playerId, itemName) {
    ...
    const nameLower = itemName.toLowerCase();
    // Try exact match first
    const exactMatch = playerItems.find((r) => r.items.name.toLowerCase() === nameLower ||
        r.items.pluralName?.toLowerCase() === nameLower);
    if (exactMatch) {
        return {
            item: toItem(exactMatch.items),
    ...
    // Try prefix match
    const prefixMatch = playerItems.find((r) => r.items.name.toLowerCase().startsWith(nameLower) ||
        r.items.pluralName?.toLowerCase().startsWith(nameLower));
    if (prefixMatch) {
        return {
            item: toItem(prefixMatch.items),
 ...
 */
function getItemDisplayName(item, quantity) {
    if (quantity === 1) {
        return item.name;
    }
 ...
 */
export async function getItem(playerId, roomId, itemName, requestedQuantity) {
    ...
    const message = quantityToTake === 1
 ...
 * @returns ItemResult with success status and message
 */
export async function dropItem(playerId, roomId, itemName, requestedQuantity) {
    ...
    const existingRoomInventory = await db
        .select()
        .from(roomInventory)
        .where(and(eq(roomInventory.roomId, roomId), eq(roomInventory.itemId, item.id)))
        .get();
    ...
    const message = quantityToDrop === 1
 ...
 */
async function findItemInContainer(containerId, itemName) {
    ...
    const nameLower = itemName.toLowerCase();
    // Try exact match first
    const exactMatch = containerItems.find((r) => r.items.name.toLowerCase() === nameLower ||
        r.items.pluralName?.toLowerCase() === nameLower);
    if (exactMatch) {
        return {
            item: toItem(exactMatch.items),
    ...
    // Try prefix match
    const prefixMatch = containerItems.find((r) => r.items.name.toLowerCase().startsWith(nameLower) ||
        r.items.pluralName?.toLowerCase().startsWith(nameLower));
    if (prefixMatch) {
        return {
            item: toItem(prefixMatch.items),
 ...
 */
async function getOpenContainersInRoom(roomId) {
    return db
        .select()
        .from(containers)
        .where(and(eq(containers.roomId, roomId), eq(containers.isOpen, true), eq(containers.isHidden, false)));
}
/**
 * Generate auto-description of item stats for private info
 */
function generateStatsDescription(item) {
    const parts = [];
    // Stat effects
    const statNames = {
        str: "STR",
    ...
    if (item.weaponDamage) {
        parts.push(`Damage: ${item.weaponDamage}${item.weaponType ? ` (${item.weaponType})` : ""}`);
    }
    // Magic properties
 ...
 */
export async function examineItem(playerId, roomId, itemName, searchOwn = false) {
    let item = null;
    let isOwnItem = false;
    if (searchOwn) {
 ...
 * @returns ItemResult with success status and message
 */
export async function getItemFromContainer(playerId, roomId, itemName, containerName, requestedQuantity) {
    // Find the container
    ...
    const container = containerList.find((c) => c.name.toLowerCase() === containerNameLower ||
 ...
 * @returns ItemResult with success status and message
 */
export async function giveItem(fromPlayerId, roomId, toPlayerName, itemName, requestedQuantity) {
    ...
    const targetPlayer = roomPlayers.find((p) => p.name.toLowerCase() === targetNameLower ||
    ...
    const existingTargetInventory = await db
        .select()
        .from(playerInventory)
        .where(and(eq(playerInventory.playerId, targetPlayer.id), eq(playerInventory.itemId, item.id)))
        .get();
    ...
    const message = quantityToGive === 1
