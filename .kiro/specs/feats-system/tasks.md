...
- [x] 1. Database schema and types
  - [x] 1.1 Add feats, feat_prerequisites, and player_feats tables to schema
  ...
  - [x] 1.2 Add unspent_feat_slots and active_stance columns to players table
    - Add `unspent_feat_slots` integer column defaulting to 0
    - Add `active_stance` text column referencing feats.id (nullable)
    - _Requirements: 4.4, 7.1_

  - [x] 1.3 Add weaponRange column to items table
    - Add `weaponRange` text column for "melee" or "ranged" values
    - _Requirements: 8.6_

  - [x] 1.4 Create feat type definitions in src/types/feat.ts
    - Define Feat, FeatPrerequisite, PlayerFeat, WeaponRange types
    - _Requirements: 1.1, 1.4_

- [x] 2. Checkpoint - Run db:reset and verify schema
  - Run `npm run db:reset` to apply schema changes
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. Prerequisite parser
  - [x] 3.1 Create prerequisiteParser.ts with parsePrerequisites function
  ...
  - [x] 3.2 Implement scaleLevel function for Pathfinder level conversion
    - Formula: `ourLevel = Math.round(pfLevel * (1 + pfLevel * 0.1))`
    - _Requirements: 2.4_

  - [x] 3.3 Implement babToLevel function for BAB to level conversion
    - Formula: `requiredLevel = babValue * 3`
    - _Requirements: BAB system_

  - [x]\* 3.4 Write property test for level scaling (Property 1)
    - **Property 1: Level Scaling Formula**
    - **Validates: Requirements 2.4**

  - [x]\* 3.5 Write property test for BAB calculation (Property 16)
    - **Property 16: BAB Calculation**
    - **Validates: BAB system**

  - [x]\* 3.6 Write unit tests for prerequisite parser
...
- [x] 4. Feat seeder
  - [x] 4.1 Create featSeeder.ts to parse CSV and seed feats
  ...
  - [x] 4.2 Seed the 6 starter feats with effect_type values
  ...
  - [x] 4.3 Update seed script to include feat seeding
    - Add feat seeder to db:seed command
    - _Requirements: 2.1_

- [x] 5. Checkpoint - Run db:reset and verify seeding
  - Run `npm run db:reset` to seed feats
  - Verify 6 starter feats exist with correct data
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. FeatService core functions
  - [x] 6.1 Create FeatService.ts with calculateBAB function
    - Formula: `BAB = floor(level / 3)`
    - _Requirements: BAB system_

  - [x] 6.2 Implement getPlayerFeats function
    - Query player_feats joined with feats table
    - Return array of Feat records
    - _Requirements: 5.1_

  - [x] 6.3 Implement getFeatByName function
    - Case-insensitive fuzzy match on feat name
    - _Requirements: 5.3_

  - [x] 6.4 Implement hasFeat function
    - Check if player has a specific feat by name
    - _Requirements: 6.6_

  - [x] 6.5 Implement checkPrerequisites function
  ...
  - [x]\* 6.6 Write property test for prerequisite evaluation (Property 3)
    - **Property 3: Prerequisite Evaluation**
    - **Validates: Requirements 3.1, 3.2, 3.3, 3.4**

  - [x] 6.7 Implement getAvailableFeats function
    - Filter by: supported status, not already owned, prerequisites met
    - _Requirements: 5.2_

  - [x]\* 6.8 Write property test for available feats filtering (Property 19)
    - **Property 19: Available Feats Filtering**
    - **Validates: Requirements 5.2**

  - [x]\* 6.9 Write property test for unsupported feat handling (Property 4)
    - **Property 4: Unsupported Feat Handling**
    - **Validates: Requirements 3.5, 3.6**

- [x] 7. Feat acquisition
  - [x] 7.1 Implement grantFeatSlot function
    - Increment unspent_feat_slots by 1
    - _Requirements: 4.2, 4.3_

  - [x] 7.2 Implement acquireFeat function
  ...
  - [x]\* 7.3 Write property test for even level feat slot grant (Property 5)
    - **Property 5: Even Level Feat Slot Grant**
    - **Validates: Requirements 4.2, 4.3**

  - [x]\* 7.4 Write property test for feat acquisition invariants (Property 6)
    - **Property 6: Feat Acquisition Invariants**
    - **Validates: Requirements 4.5, 4.7**

  - [x]\* 7.5 Write property test for zero slots rejection (Property 7)
    - **Property 7: Zero Slots Rejection**
    - **Validates: Requirements 4.6**

  - [x]\* 7.6 Write property test for unique player-feat constraint (Property 8)
    - **Property 8: Unique Player-Feat Constraint**
    - **Validates: Requirements 1.3**

- [x] 8. Checkpoint - Verify FeatService core
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. Category and filtering functions
  - [x] 9.1 Implement getFeatsByCategory function
    - Query feats by category with optional supportability filter
    - _Requirements: 10.3_

  - [x] 9.2 Implement getCategories function
    - Return unique categories with feat counts
    - _Requirements: 10.2_

  - [x]\* 9.3 Write property test for category filtering (Property 18)
    - **Property 18: Category Filtering**
    - **Validates: Requirements 10.3**

- [x] 10. Stance system
  - [x] 10.1 Implement isStanceFeat function
    - Check if feat has effect_type = "combat_stance"
    - _Requirements: 7.5_

  - [x] 10.2 Implement setActiveStance function
    - Validate player has the stance feat
    - Validate feat is a stance feat
    - Update players.active_stance to feat_id or null
    - _Requirements: 7.2, 7.3, 7.4, 7.5_

  - [x] 10.3 Implement getActiveStance function
    - Return active stance feat or null
    - _Requirements: 7.6_

  - [x]\* 10.4 Write property test for single active stance (Property 15)
    - **Property 15: Single Active Stance**
    - **Validates: Requirements 7.4**

- [x] 11. FeatEffectHandler
  - [x] 11.1 Create FeatEffectHandler.ts with getToughnessBonus function
    - Formula: +3 HP, +1 HP per level beyond 3rd
    - Return 0 if player doesn't have Toughness
    - _Requirements: 6.2_

  - [x]\* 11.2 Write property test for Toughness HP bonus (Property 10)
    - **Property 10: Toughness HP Bonus**
    - **Validates: Requirements 6.2**

  - [x] 11.3 Implement getACModifiers function
    - Return dodge bonus (+1 if player has Dodge)
    - Return stance bonus (+1 if Combat Expertise active)
    - _Requirements: 6.3, 6.4_

  - [x]\* 11.4 Write property test for Dodge AC bonus (Property 11)
    - **Property 11: Dodge AC Bonus**
    - **Validates: Requirements 6.3**

  - [x]\* 11.5 Write property test for Combat Expertise stance effects (Property 12)
    - **Property 12: Combat Expertise Stance Effects**
    - **Validates: Requirements 8.1**

  - [x] 11.6 Implement getAttackModifiers function
    - Return -1 if any stance is active
    - _Requirements: 6.4, 8.1, 8.2, 8.3_

  - [x] 11.7 Implement getDamageModifiers function
    - Return +2 if Power Attack active and weapon is melee
    - Return +2 if Deadly Aim active and weapon is ranged
    - Return 0 otherwise
    - _Requirements: 8.2, 8.3, 8.4, 8.5_

  - [x]\* 11.8 Write property test for Power Attack stance effects (Property 13)
    - **Property 13: Power Attack Stance Effects**
    - **Validates: Requirements 8.2, 8.4**

  - [x]\* 11.9 Write property test for Deadly Aim stance effects (Property 14)
    - **Property 14: Deadly Aim Stance Effects**
    - **Validates: Requirements 8.3, 8.5**

  - [x] 11.10 Implement canDualWield function
    - Return true if player has Two-Weapon Fighting feat
    - _Requirements: 9.1_

  - [x]\* 11.11 Write property test for Two-Weapon Fighting validation (Property 17)
    - **Property 17: Two-Weapon Fighting Validation**
    - **Validates: Requirements 9.1, 9.2**

- [x] 12. Checkpoint - Verify FeatEffectHandler
  - Ensure all tests pass, ask the user if questions arise.

- [x] 13. Service integrations
  - [x] 13.1 Integrate FeatEffectHandler into StatService for HP calculation
    - Call getToughnessBonus in calculateMaxHp
    - _Requirements: 6.2_

  - [x] 13.2 Integrate FeatEffectHandler into StatService for AC calculation
    - Call getACModifiers in calculateAC
    - _Requirements: 6.3, 6.4_

  - [x] 13.3 Integrate FeatEffectHandler into CombatService for attack rolls
  ...
  - [x] 13.4 Integrate FeatService into EquipmentService for dual wield validation
    - Call canDualWield before allowing offHand weapon equip
    - Return error message if player lacks Two-Weapon Fighting
    - _Requirements: 9.1, 9.2, 9.3_

  - [x] 13.5 Integrate feat slot granting into level-up logic
    - Call grantFeatSlot when player reaches even levels
    - _Requirements: 4.2, 4.3_

- [x] 14. Checkpoint - Verify service integrations
  - Ensure all tests pass, ask the user if questions arise.

- [x] 15. Command handlers
  - [x] 15.1 Create feats command handler
  ...
  - [x]\* 15.2 Write property test for feat slot display (Property 20)
    - **Property 20: Feat Slot Display**
    - **Validates: Requirements 5.6**

  - [x] 15.3 Create stance command handler
    - `stance` - show current stance with hint
    - `stance <name>` - activate stance
    - `stance off` - deactivate stance
    - _Requirements: 7.6, 7.7, 7.8_

  - [x] 15.4 Update stats command to show active stance
    - Display stance name and hint for deactivation
    - _Requirements: 7.9_

  - [x]\* 15.5 Write unit tests for command handlers
    - Test feats command variations
    - Test stance command variations
    - Test stats display with stance
    - _Requirements: 5.1-5.7, 7.6-7.9_

- [x] 16. Checkpoint - Verify commands
  - Ensure all tests pass, ask the user if questions arise.

- [x]\* 17. Database integrity tests
  - [x]\* 17.1 Write property test for cascade delete (Property 9)
